...
export const Home = () => {
  ...
  const filteredMovies = searchValue
    ? allFilms.filter((movie) => {
        return movie.title.toLowerCase().includes(searchValue.toLocaleLowerCase());
      })
    : films;
  ...
  return (
    <div className="home">
      <header>
        <NavBar onClick={handleOpenMenu} value={<List />} />
        <NavMobile onClick={handleOpenMenu} value={<XLg />} />
      </header>
      <section className="films">
        {!!searchValue && <ParagraphMessage message={'Search: ' + searchValue} />}

        <InputSearch onChange={handleChange} value={searchValue} />

        {filteredMovies.length > 0 && <Films films={filteredMovies} />}

        {filteredMovies.length === 0 && <ParagraphMessage message="the movie doesn´t exist" />}

        {!searchValue && <ButtonPage onClick={loadMoreFilms} disabled={noMoreFilms} />}
      </section>
    </div>
  );
};
